...
FEATURE: Collapsed sidebar active workspace gets a 2px accent left stripe with accent_glow halo — signature marker from the mockup, replicates at rail scale the accent-stripe rule used by tab bar and command palette.
FEATURE: Sidebar rail refresh — collapsed mode now matches the "Luminous Void" spec: 32x32 rounded squares (was 20x20 circles) with numbered workspace labels rendered via glyphon, `surface_2` fill for active/hover, right-edge `accent_glow` luminous separator, logo cell with `w` mark at the top (46px offset), notification indicators wired (`unread_count == 1` → 7px accent dot, `>= 2` → 15px accent badge with inverse text). Collapsed is the first-launch default (`is_none_or` fallback in session restoration) so the rail is what users see out of the box; existing sessions keep their last state. Tests updated to explicitly opt into expanded mode.
FEATURE: Command palette responds to mouse-wheel scrolling when the cursor is over its rect — 3 rows per LineDelta notch, PixelDelta divided by `RESULT_HEIGHT`, intercepted before the terminal scroll handler so it does not also scroll the underlying pane. Wheel scroll is independent of `selected` (standard palette UX: arrow keys move selection, wheel moves the viewport). Implemented via `palette_last_selected` in `UiState`: the render loop only calls `scroll_to_selected` when selection changed since last frame, so a wheel-set `scroll_offset` is not snapped back. `MouseWheel` branch added in `event_loop.rs` above the notification-panel branch.
FIX: Command palette "All" filter no longer identical to "Commands" — workspaces and surfaces were previously dropped because `palette_actions`/`palette_row_sections`/`palette_row_icons` were being truncated at `MAX_VISIBLE_RESULTS=20` before storage (with 22 commands + 8 sub-category headers, everything past the first 20 rows was discarded). Rows now store the full list; added `CommandPalette.scroll_offset` + `scroll_to_selected(visible, total)` helper that shifts the viewport every frame to keep the selected row visible, with `palette_rows_cache` backing repaint when only `scroll_offset` changed (no search re-run). Selected-row highlight in `CommandPalette::render_quads` now positions from `selected - scroll_offset`, workspace-square + text-area loops look up `palette_actions[scroll_offset + slot]` so icons/colors match the visible window. Sentinel resets clear `scroll_offset` on open/close/filter-change.
FEATURE: Command palette groups results by thematic sub-category when query is empty — SPLIT / PANE / FOCUS / WORKSPACE / TABS / EDIT / VIEW / SEARCH / OTHER section headers inserted between groups via `command_category(id)` mapping in `render.rs`. Query-active view keeps the single "COMMANDS — MATCHING «query»" header per the mockup. Navigation and Enter already skip section rows through `palette_row_sections`, no changes needed there.
FEATURE: Command palette gains per-row Codicons icons (Split, Close, ArrowUp/Down/Left/Right, Add, Globe, Search, Workspace, Terminal) rendered in a 32px left column via existing `prepare_with_custom` SVG pipeline; workspace rows replace the glyph with a 22x22 rounded colored square (accent/success/amber/accent_hi cycled by workspace index) showing the number in `text_inverse`; new 36px footer row at bottom with `↑↓ navigate / ↵ select / Tab filter / ESC close` mono hints at left and a 10px accent brand mark + "wmux" text at right; `PaletteLayout.total_height` bumped to include footer, visible rows cap adjusted so the palette still fits short windows.
FEATURE: Command palette result rows carry a description line — `CommandEntry.description` (previously unused in the palette) renders as a muted 12px caption below the 13px name. Workspace rows use that slot for a `branch · N panes · cwd` meta line built from `WorkspaceSnapshot`. Filter=All now inserts `COMMANDS — MATCHING "…"`, `WORKSPACES`, `SURFACES` section headers between groups; navigation (↑/↓) and Enter skip section rows via a parallel `palette_row_sections` vec. Row height 36→44px to fit the two-line layout. `palette_desc_buffers` pre-allocated at init like the other palette buffer pools.
